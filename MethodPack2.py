...
from MethodPack1 import plot_distribution
#===================
#  I have cahnge the getDM_ReplicaToNode() method in MDGraophClass to getDM_ReplicaToNode()
...
def Return_sorted_DMs_InList_BasedOn_Dependencies_and_ResourceUtilization(inst):  # list of DMs for ech instance
    ...
    for DM, Depend, in zip(inst.DMGraphs, inst.DM_Dependencies):
        if DM.getDM_ReplicaToNode() == -1 and DM.getDM_type()!=0:#return all DM that not migrated and not staefull
            listOf_potintioalDMs.append(
                [DM.getDM_ID(), DM.CPU_Utilization, DM.Memory_Utilization, DM.getDM_ReplicaToNode()])
...
def return_SUM_of_cpu_Mem_utilization(node):
    ...
    for j in node.instances:
        if node.node.getID() == j.getmigrating_ToMachine_Number() or j.getmigrating_ToMachine_Number() == -1:
            CPUUti.append(j.getCPU()*2)
            MemUti.append(j.getMem()*8)
            #node.getCPU()*128
            #node.getMem() * 512
    return 128-(sum(CPUUti)), 512-(sum(MemUti))
    #return node.node.getCPU()*128,node.node.getMem()*512
        ...
        for inst in s.instances:
            if (s.node.getMachineID() == machineNumber and inst.getmigrating_ToMachine_Number() == -1) or (
                ...
                for dm in inst.DMGraphs:
                    if (dm.getDM_MachineId() == machineNumber and dm.getDM_ReplicaToNode() == -1) or (
                            dm.getDM_ReplicaToNode() != -1 and m.getDM_ReplicaToNode() == machineNumber):
...
def return_list_of_Potentioal_Machines(ListOfmachines, machine_number):
    ...
    for i in ListOfmachines:
        if i.getpodNumber() == pod_number and i.getMachID() != machine_number:  # or i.getswitchNumber()==switch_number:# and i.getMachID()!=machine_number:
        ...
        for s3 in schedul:
            for inst3 in s3.instances:
                for DM in inst3.DMGraphs:
                    if DM.getDM_ReplicaToNode() == s.node.getMachineID() or DM.getDM_MachineId() == s.node.getMachineID():
